...
public class InputStudentFormController implements ActionListener {
    ...
    public void actionPerformed(ActionEvent event)  {
        ...
        if (event.getActionCommand().equals("Save")) {
            try {
                LOGGER.log(Level.INFO,"Saving data for add a student");
                this.saveData();
            } catch (InvalidDataException ex) {
                LOGGER.log(Level.INFO,"A problem occured when creating a student");
                ErrorPopup error = new ErrorPopup(form, new InvalidDataException("A problem occured when creating a student: " + ex));
            }
        } else if (event.getActionCommand().equals("Clear")) {
            LOGGER.log(Level.INFO,"Clearing the add a student form");
            this.clearForm();
        } else if (event.getActionCommand().equals("Close")) {
            LOGGER.log(Level.INFO,"Closing the add a student form");
            this.closeForm();
        }
    }
     ...
     * object
     */
    public void saveData() throws InvalidDataException {
        LOGGER.setLevel(Level.ALL); 
        ...
        try {
            ...
            LOGGER.log(Level.INFO,"NULL GPA, creating a student with GPA 0");
            newStudent.setGPA(0);           
            ErrorPopup error = new ErrorPopup(form, new InvalidDataException("NULL GPA, creating a student with GPA 0"));
        }
        ...
        try {
            ...
            LOGGER.log(Level.INFO,"A problem occured when saving dates inside StudentFormController");
            ErrorPopup error = new ErrorPopup(form, new InvalidDataException("A problem occured when saving dates inside StudentFormController"));
        }
